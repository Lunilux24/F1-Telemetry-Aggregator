...
load_dotenv()
DEFAULT_JOLPICA_URL = os.environ.get("JOLPICA_URL", "http://api.jolpi.ca/ergast/f1/")
...
def parse_args():
    ...
    p.add_argument('--source', default='jolpica', help='source name used in S3 key (default: jolpica)')
    p.add_argument('--url', default=DEFAULT_JOLPICA_URL,
    ...
    for attempt in range(1, args.retries + 1):
        try:
            if args.mock_file:
                ...
                logging.info(f"Fetching data from: {args.url}")
                body = fetch_url(session, args.url)
            
            date = datetime.now(timezone.utc).strftime('%Y-%m-%d')
            ts = safe_timestamp()
            key = f"raw/{date}/{args.source}/{ts}.json"
            metadata = {'source': args.source, 'ingest_time_utc': str(ts)}
            logging.info("Uploading to s3://%s/%s", args.bucket, key)
            write_to_s3(args.bucket, key, body, region=args.region, metadata=metadata)
            logging.info("Upload OK. Key=%s", key)
            print(key)   # Jenkins console will capture this; easy for later steps
            return 0
